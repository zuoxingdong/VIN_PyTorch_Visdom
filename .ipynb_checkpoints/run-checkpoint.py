...
for epoch in range(args.epochs): # Loop over dataset multiple times
    ...
    for i, data in enumerate(trainloader):
        ...
        if X.size()[0] != args.batch_size: # TODO: Bug with DataLoader
            continue # Drop those data, no enough for a batch
    ...
    # Print epoch logs
    print('[Epoch # {:3d} ({:.1f} s)] Loss: {:.4f}'.format(epoch, time_duration, np.mean(running_losses)))
...
for i, data in enumerate(testloader):
    ...
    # Convert prediction and labels to Numpy array
    predicted = predicted.data

    # Compute test accuracy
    correct += (predicted == labels).sum()
    total += labels.size()[0] # args.batch_size, TODO: Check if DataLoader drop rest examples less than batch_size
...
print('\nRecording reward and value images.\n')

# Data index in test set
idx = 0
...
# Process reward image and value images
imgs = np.stack([net.reward_image] + net.value_images)
